...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        pic_b_path = pathlib.Path(pic_b)
        if pic_b_path.is_dir():
            pic_bs = list(pic_b_path.glob("*.jpg")) + list(pic_b_path.glob("*.png")) + list(pic_b_path.glob("*.jpeg"))
        elif pic_b_path.is_file():
            pic_bs = [pic_b_path]

        for pic_b_p in pic_bs:
            try:
                pic_b = str(pic_b_p)
                ...
                reverse2wholeimage(b_align_crop_tenor_list,swap_result_list, b_mat_list, crop_size, img_b_whole, logoclass, \
                    os.path.join(opt.output_path, pic_b_p.name), opt.no_simswaplogo,pasring_model =net,use_mask=opt.use_mask, norm = spNorm)
                print(' ')

                print('************ Done ! ************')
            except Exception as e:
                print(f"Unable to work with {pic_b_p}")
